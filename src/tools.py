...
def create_calculator_tool(logger: ToolLogger):
    ...
    - Accepts a mathematical expression as input (string)
    - Validates it for safety (only basic math)
    - Evaluates it using eval() in a restricted environment
    - Logs tool usage
    - Returns formatted result string
    - Handles errors gracefully
    """

    # Sólo permitimos aritmética básica
    ...
    def _validate_expression(expr:str)->None:
        """
        Parsea y valida expresiones AST para asegurarse de que contiene sólo 
        operaciones matemáticas permitidas y constantes numéricas
        """
        try:
    ...
    def calculator(expression: str)-> str:
        """
        Evalúa expresiones matemáticas básicas de forma segura
         - input: expression(string), e.g. "1000 * 0.25 +12"
         - output: string formateado con reult
        """

        expr= (expression or "").strip()

        try: 
            logger.log_tool_use("calculator", {"expression": expr})
        except Exception:
            pass

        if not expr:
            return "Error: empty expression. Provide a math expression like '2 + 2'."

        try:
            result = _safe_eval(expr)

            # Normalizamos
            if isinstance(result, float) and abs(result)== 0.0:
                result= 0.0
            ...
            return f"Error: failed to evaluate expression. Details: {str(e)}"
